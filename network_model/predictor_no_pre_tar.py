...
class Predictor(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        predict_dim = (cfg.horizon + 1) * cfg.state_dim + cfg.target_dim
    ...
    def predict(self, state, previous_state, next_target):
        """
        ...
        x = torch.cat([previous_state, state, next_target], dim=-1)
    ...
    def update(self, container: Container or Buffer, base_predictor=None):
        ...
        target_sequence = target_sequence[:, :(2 * 20 + 1), :].to(self.device).requires_grad_(False)
        """
        None grad data: 
        ...
        state = state_sequence[:, 2 * self.horizon, :]
        previous_state = state_sequence[:, 0:(2 * self.horizon):2, :]
        ...
        for k in range(self.train_horizon):
            next_target = target_sequence[:, 2*(self.horizon + k + 1), :]

            next_state_predict, delta_state_predict = self.predict(state, previous_state, next_target)
            next_state = state_sequence[:, 2*(self.horizon + k + 1), :]
            ...
            #         state, previous_state, next_target))
