...
class RLEnv:
    def __init__(self):
        # self.action_space = spaces.Box(low=-1, high=1, shape=(4,), dtype=np.float64)  # 动作量4维
        ...
        self.ref = np.load(os.path.dirname(__file__) +
                           '/ref_traj/zc_interp_npy_train/zc_interp_train' + self.random + '.npy')
        self.idx = 0
    ...
    def reset(self, ref_traj=None, options='train'):
        if ref_traj is None:
            self.random = str(np.random.randint(1, 41))
        else:
            self.random = str(ref_traj)
        self.ref = np.load(os.path.dirname(__file__) +
                           '/ref_traj/zc_interp_npy_' + options + '/zc_interp_' + options + self.random + '.npy')
        self.idx = 0
        ...
        self.data_save = {"target": [], "real_target": [], "js": [], "jv": [], "time": [], "action": []}
        print("\nreset!")
        print("train ref" + self.random)

        return self.get_state(np.array([0.0, 0.0, 0.0, 0.0]))

    def get_reward(self):
        # PPO
        reward = 100 * (1.0 - np.sqrt(np.mean(np.square(self.state[:4] - self.ref[self.idx, :]))))  # MSRE
        reward += 10 * (- np.sqrt(np.mean(np.square(self.state[:4] - self.previous_state[-8:-4]))))
        # SAC
        # reward = 100 * (0.2 - np.sqrt(np.mean(np.square(self.state[:4] - self.ref[self.idx, :]))))  # MSRE
        # reward += 100 * (0.2 - np.sqrt(np.mean(np.square(self.state[:4] - self.previous_state[-8:-4]))))
        return reward
    ...
    def save_test(self):
        ...
        os.makedirs(os.path.dirname(__file__) + '/test_data_test', exist_ok=True)
        np.save(os.path.dirname(__file__) + '/test_data_test/{}_{}.npy'.format(file_name, timestamp), self.data_save)
        df = pd.DataFrame(self.data_save)
        csv_file = os.path.dirname(__file__) + '/test_data_test/{}_{}.csv'.format(file_name, timestamp)
        df.to_csv(csv_file, index=False)
        print('\nsaved', time.localtime())
