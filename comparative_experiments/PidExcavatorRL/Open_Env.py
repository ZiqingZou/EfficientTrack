...
class RLEnv:
    def __init__(self):
        ...
        self.ref = np.load('../ref_traj/zc_interp_npy_train/zc_interp_train' + self.random + '.npy')
        self.idx = 0
    ...
    def reset(self, ref_traj=0, options='train'):
        ...
        self.ref = np.load('../ref_traj/zc_interp_npy_' + options + '/zc_interp_' + options + self.random + '.npy')
        self.idx = 0
    ...
    def get_reward(self, a):
        # PPO
        reward = 5.0 * (0.1 - np.sqrt(np.mean(np.square(self.state[:4] - self.ref[self.idx, :]))))
        reward += 5.0 * (0.1 - np.sqrt(np.mean(np.square(a - self.last_action))))  # MSRE
        # SAC
